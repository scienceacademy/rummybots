...
class felix(Bot):
    ...
    def draw_decision(self, view: PlayerView) -> str:
        ...
        if view.top_of_discard is None:
            return "deck"
        if calculate_deadwood(view.hand) - evaluate_discard_draw(view.hand, view.top_of_discard) > 3:
            # or 2
    ...
    def discard_decision(self, view: PlayerView) -> Card:
        ...
        valid_discards = [c for c in hand if c != self._drew_from_discard]

        unmelded_valid = [c for c in unmelded if c in valid_discards]

        if unmelded_valid:
            unmelded_ranks = [c.rank for c in unmelded]

            # useless
            safe_to_discard = []
            for c in unmelded_valid:
                # pairs
                pair = unmelded_ranks.count(c.rank) >= 2

                # related/straight thing
                connected = any(
                    other.suit == c.suit and abs(other.rank.value - c.rank.value) == 1
                    for other in unmelded if other != c
                )

                if not pair and not connected:
                    safe_to_discard.append(c)

            if safe_to_discard:
                # highest
                return max(safe_to_discard, key=lambda c: c.deadwood_value)

            # highest card anyway
            return max(unmelded_valid, key=lambda c: c.deadwood_value)
        # backup
        return best_discard(valid_discards)
    ...
    def knock_decision(self, view: PlayerView) -> bool:
        ...
        if deadwood == 0:
            return True
        # elif view.deck_size > 28:
        #    if deadwood <= 10:
        #        return True
        # elif view.deck_size > 21:
        #    if deadwood <= 4:
        #        return True
        # return deadwood <= 2
        if view.deck_size < 10:
            return deadwood <= 10
        elif view.deck_size < 25:
            return deadwood <= 6
        else:
            return deadwood <= 3
