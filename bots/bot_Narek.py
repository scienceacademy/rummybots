...
class StudentBot(Bot):
    ...
    def draw_decision(self, view: PlayerView) -> str:
        ...
        if view.top_of_discard is not None:
            ...
            deadwood_if_take = evaluate_discard_draw(view.hand, view.top_of_discard)

            if deadwood_if_take < current_deadwood:
                self._last_discard_draw = view.top_of_discard
                return "discard"

        self._last_discard_draw = None
        return "deck"
    ...
    def discard_decision(self, view: PlayerView) -> Card:
        ...
        """
               # Prevent illegal discard of card just taken from discard pile
        if hasattr(self, "_last_discard_draw") and self._last_discard_draw:
            valid_cards = [
                c for c in view.hand
                if c != self._last_discard_draw
            ]
        else:
            valid_cards = view.hand

        return best_discard(valid_cards)
    ...
    def knock_decision(self, view: PlayerView) -> bool:
        ...
        # Late game: knock whenever allowed
        return deadwood <= 6