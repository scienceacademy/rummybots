...
class BucaneerBot(Bot):
    ...
    def draw_decision(self, view: PlayerView) -> str:
        ...
        if calculate_deadwood(view.hand) <= 13:
    ...
    def discard_decision(self, view: PlayerView) -> Card:
        choices = view.hand
        unmelded = get_unmelded_cards(view.hand)
    ...
    def knock_decision(self, view: PlayerView) -> bool:
        ...
        if calculate_deadwood(view.hand) == 0:
            return True
        if view.deck_size >= 40:
            return True
        if (40 > view.deck_size >= 20) and (calculate_deadwood(view.hand) < 5):
            return True
